...
export default function CartDrawer() {
    ...
    return (
        <div style={{
            ...
            background: 'var(--color-bg-secondary)', zIndex: 2000,
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
                {cart.length === 0 ? (
                    <p style={{ textAlign: 'center', color: 'var(--color-text-muted)' }}>Your cart is empty.</p>
                ) : (
                    cart.map(item => (
                        <div key={item.id} style={{ display: 'flex', marginBottom: '1.5rem', gap: '1rem' }}>
                            <div style={{ width: '60px', height: '60px', background: 'var(--color-bg-secondary)', borderRadius: '4px' }}>
                                {item.images && <img src={item.images} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                            </div>
                            <div style={{ flex: 1 }}>
                                <h4 style={{ marginBottom: '0.25rem' }}>{item.name}</h4>
                                <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)', marginBottom: '0.5rem' }}>₹{item.price}</p>
...
const qtyBtnStyle = {
    width: '24px', height: '24px', borderRadius: '50%', border: '1px solid var(--color-border)', background: 'var(--color-bg-secondary)', cursor: 'pointer'
};
