    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
                ...
                {/* Form */}
                <div style={{ background: 'var(--color-bg-secondary)', padding: '1.5rem', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', height: 'fit-content' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }}>
                        <div>
                            ...
                            <input type="number" required value={formData.amount} onChange={e => setFormData({ ...formData, amount: e.target.value })} style={{ width: '100%', padding: '0.75rem', border: '1px solid var(--color-border)', borderRadius: '4px' }} />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" required value={formData.reason} onChange={e => setFormData({ ...formData, reason: e.target.value })} placeholder="e.g. Server Cost, Packaging" style={{ width: '100%', padding: '0.75rem', border: '1px solid var(--color-border)', borderRadius: '4px' }} />
                        </div>
                        <div>
                            <label style={{ fontSize: '0.9rem', display: 'block', marginBottom: '0.25rem' }}>Date</label>
                            <input type="date" required value={formData.date} onChange={e => setFormData({ ...formData, date: e.target.value })} style={{ width: '100%', padding: '0.75rem', border: '1px solid var(--color-border)', borderRadius: '4px' }} />
                        </div>
                ...
                <div>
                    <div style={{ background: 'var(--color-black)', color: 'white', padding: '1.5rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
                        <h4 style={{ color: 'var(--color-text-muted)', margin: '0 0 0.5rem 0', textTransform: 'uppercase', fontSize: '0.8rem' }}>Total Expenses</h4>
                        <p style={{ fontSize: '2rem', fontWeight: 'bold', margin: 0, color: 'var(--color-gold)' }}>₹{total.toLocaleString()}</p>
                    </div>

                    <div style={{ background: 'var(--color-bg-secondary)', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                        <h4 style={{ padding: '1rem', margin: 0, borderBottom: '1px solid var(--color-border)' }}>Recent Logs</h4>
                            ...
                            {expenses.map(exp => (
                                <div key={exp.id} style={{ padding: '1rem', borderBottom: '1px solid #f9f9f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        ...
                                        <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>{new Date(exp.date).toLocaleDateString()} • {exp.investorName}</p>
                                    </div>
